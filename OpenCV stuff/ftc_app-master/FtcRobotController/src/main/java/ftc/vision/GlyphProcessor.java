 ...
 * FTC Team EV 7393
 */
public class GlyphProcessor implements ImageProcessor<GlyphColorResult> {
    private static final String TAG = "BeaconProcessor";
    private static final double MIN_MASS = 6;

    @Override
    public ImageProcessorResult<GlyphColorResult> process(long startTime, Mat rgbaFrame, boolean saveImages) {
        ...
        hsvMin.add(new Scalar(330/2,  36, 62)); //Brown min
        hsvMax.add(new Scalar( 16/2, 218, 120)); //Brown max

        hsvMin.add(new Scalar( 200/2,  0, 90)); //Gray min
        hsvMax.add(new Scalar(300/2, 55, 195)); //Gray max
        ...
        GlyphColorResult.GlyphColor[] beaconColors = GlyphColorResult.GlyphColor.values();
        GlyphColorResult.GlyphColor left = beaconColors[maxMassIndex[0]];
        GlyphColorResult.GlyphColor right = beaconColors[maxMassIndex[1]];
        ...
        return new ImageProcessorResult<>(startTime, rgbaFrame,
                new GlyphColorResult(left, right)
        );
    }
}
